...
**变量1：TJD\_TOKEN（账号Token）**

- 名称：`TJD_TOKEN`
- 值：运行 `get_token.py` 获取的 JSON 内容

**变量2：TJD\_API（青龙API配置，用于自动更新Token）**
...
| 变量名         | 说明                           |
| ----------- | ---------------------------- |
| `TJD_TOKEN` | 账号 Token，运行 get\_token.py 获取 |
...
- **未配置 TJD\_API**：重新运行 `get_token.py` 获取新 Token，手动更新青龙环境变量
- **已配置 TJD\_API**：脚本会自动更新 Token，无需手动操作
...
- [NTE-Auto-Sign-main](https://github.com/Candy-QAQ/NTE-Auto-Sign) - 原始签到逻辑